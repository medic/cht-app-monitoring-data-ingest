CREATE OR REPLACE VIEW app_monitoring_gateway_health AS (
    SELECT
        ...
        CASE
            WHEN
                metrics.due > 0 AND metrics.delivered = 0 THEN 'Gateway Unhealthy'
            ELSE
                'Gateway Healthy'
            ...
            WHEN metrics.due > 0 AND metrics.delivered = 0 THEN
                (
                    SELECT
                        a.reported
                    FROM
                        get_last_sms_metric() AS a
                    WHERE
                        a.reported < metrics.reported AND a.delivered > 0 AND a.partner_name = metrics.partner_name
                    LIMIT 1
                )
            ELSE
                metrics.reported
    ...
    FROM
        get_last_sms_metric() AS metrics
);
